...
export const productManagementService = {
  ...
  async createProduct(productData) {
    try {
      ...
      return await httpClient.post('/api/thiyen/products', productData)
    } catch (error) {
      if (shouldUseMock()) {
        return {
          success: true,
  ...
  async updateProduct(id, productData) {
    try {
      ...
      return await httpClient.put(`/api/thiyen/products/${id}`, productData)
    } catch (error) {
      if (shouldUseMock()) {
        return {
          success: true,
  ...
  async deleteProduct(id) {
    try {
      ...
      return await httpClient.delete(`/api/thiyen/products/${id}`)
    } catch (error) {
      if (shouldUseMock()) {
        return {
          success: true,
